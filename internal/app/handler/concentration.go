...
func (h *Handler) GetConcentration(c *gin.Context) {
	...
	calc, items, err := h.Repo.GetConcentrationByID(id)
	if err != nil {
		//c.String(http.StatusNotFound, "Concentration not found")
		c.Redirect(http.StatusFound, "/")
		return
	}
	// Проверяем, не удалена ли заявка (статус "удалён") – если да, возвращаем 404 или сообщение
	if calc.Status == "удалён" {
		//c.String(http.StatusNotFound, "Concentration was deleted")
		c.Redirect(http.StatusFound, "/")
		return
	}
	c.HTML(http.StatusOK, "concentration.html", gin.H{
		"concentration": calc,
		"items":       items,
	})
}

func (h *Handler) AddToConcentration(c *gin.Context) {
	...
	// Добавляем раствор
	err = h.Repo.AddElectrolyteToConcentration(draft.ID, uint(electrolyteID), volume, comment)
	if err != nil {
	...
	c.Redirect(http.StatusFound, "/concentration/"+strconv.Itoa(int(draft.ID)))
}

func (h *Handler) DeleteConcentration(c *gin.Context) {
	calcIDStr := c.PostForm("concentration_id")
	calcID, err := strconv.Atoi(calcIDStr)
	if err != nil {
		//c.String(http.StatusBadRequest, "Invalid concentration ID")
		c.Redirect(http.StatusFound, "/")
		return
	}

	// Логическое удаление через SQL UPDATE (без ORM)
	err = h.Repo.DeleteConcentration(uint(calcID))
	if err != nil {
		h.errorJSON(c, http.StatusInternalServerError, err)
		return
	}
